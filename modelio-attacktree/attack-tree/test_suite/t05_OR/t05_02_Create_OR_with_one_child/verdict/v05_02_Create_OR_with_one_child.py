...
def main():
	...
	testPackage = findPackage(modelPackage, "t05_OR", "t05_02_Create_OR_with_one_child")
	...
	if(tree.getOwnedElement().isEmpty()):
		outputError("/errors_output/t05_02_Create_OR_with_one_child.err", "Expected to find OR node, instead we found only Root \n")
	else: 
		orNode = tree.getOwnedElement().get(0)
		if(orNode.getName() != "OR"):
			outputError("/errors_output/t05_02_Create_OR_with_one_child.err", "Expected to find node with the name 'OR', instead we found the name " + orNode.getName() + "\n")
		if(orNode.getMClass().getName() != "Class"):
			outputError("/errors_output/t05_02_Create_OR_with_one_child.err", "Expected to find a the meta class Class, instead we found " + orNode.getMClass().getName() + "\n")
		if(orNode.getExtension().get(0).getName() != "OR"):
			outputError("/errors_output/t05_02_Create_OR_with_one_child.err", "Expected to find a stereotype with the name 'OR', instead we found the name " + orNode.getExtension().get(0).getName() + "\n")
		diagramService = Modelio.getInstance().getDiagramService()
		diagramHandle = diagramService.getDiagramHandle(diagram)
		if(diagramHandle.getDiagramGraphics(orNode).isEmpty()) :
			outputError("/errors_output/t05_02_Create_OR_with_one_child.err", "The created OR node is not represented in the diagram !\n")
		
		if(orNode.getOwnedElement().isEmpty()):
			outputError("/errors_output/t05_02_Create_OR_with_one_child.err", "Expected to an attack child of 'OR'\n")
		else :
			attack = orNode.getOwnedElement().get(0)
			if(attack.getName() != "Attack"):
				outputError("/errors_output/t05_02_Create_OR_with_one_child.err", "Expected to find node with the name 'Attack', instead we found the name " + attack.getName() + "\n")
			diagramService = Modelio.getInstance().getDiagramService()
			diagramHandle = diagramService.getDiagramHandle(diagram)
			if(diagramHandle.getDiagramGraphics(attack).isEmpty()) :
				outputError("/errors_output/t05_02_Create_OR_with_one_child.err", "The attack child of OR node is not represented in the diagram !\n")
