...
def main():
	...
	testPackage = findPackage(modelPackage, "t05_OR", "t05_01_Create_OR_with_no_children")
	...
	if(tree.getOwnedElement().isEmpty()):
		outputError("/errors_output/t05_01_Create_OR_with_no_children.err", "Expected to find OR node, instead we found only Root \n")
	else: 
		orNode = tree.getOwnedElement().get(0)
		if(orNode.getName() != "OR"):
			outputError("/errors_output/t05_01_Create_OR_with_no_children.err", "Expected to find node with the name 'OR', instead we found the name " + orNode.getName() + "\n")
		if(orNode.getMClass().getName() != "Class"):
			outputError("/errors_output/t05_01_Create_OR_with_no_children.err", "Expected to find a the meta class Class, instead we found " + orNode.getMClass().getName() + "\n")
		if(orNode.getExtension().get(0).getName() != "OR"):
			outputError("/errors_output/t05_01_Create_OR_with_no_children.err", "Expected to find a stereotype with the name 'OR', instead we found the name " + orNode.getExtension().get(0).getName() + "\n")
		diagramService = Modelio.getInstance().getDiagramService()
		diagramHandle = diagramService.getDiagramHandle(diagram)
		if(diagramHandle.getDiagramGraphics(orNode).isEmpty()) :
			outputError("/errors_output/t05_01_Create_OR_with_no_children.err", "The created OR node is not represented in the diagram !\n")
