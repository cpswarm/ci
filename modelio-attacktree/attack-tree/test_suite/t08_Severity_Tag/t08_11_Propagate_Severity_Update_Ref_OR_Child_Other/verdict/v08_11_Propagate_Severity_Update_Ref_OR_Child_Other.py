...
def main():
	...
	testPackage = findPackage(modelPackage, "t08_Severity_Tag", "t08_11_Propagate_Severity_Update_Ref_OR_Child_Other")
	...
	if(tree.getName() != "Tree"):
		outputError("/errors_output/t08_11_Propagate_Severity_Update_Ref_OR_Child_Other.err", "Expected to find tree with the name 'Tree', instead we found the name " + tree.getName() + "\n")
	if tree.getTag("AttackTreeDesigner", "Attack", "Severity").getActual().get(0).getValue() != "HIGH" :
		outputError("/errors_output/t08_11_Propagate_Severity_Update_Ref_OR_Child_Other.err", "Expected to find Severity Tag with the value 'HIGH', instead we found the value " + attack.getTag("AttackTreeDesigner", "Attack", "Severity").getActual().get(0).getValue() + "\n")
