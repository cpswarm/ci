...
def main():
	...
	testPackage = findPackage(modelPackage, "t08_Severity_Tag", "t08_09_Propagate_Severity_Update_OR_Child")
	...
	if(tree.getName() != "Tree"):
		outputError("/errors_output/t08_09_Propagate_Severity_Update_OR_Child.err", "Expected to find tree with the name 'Tree', instead we found the name " + tree.getName() + "\n")
	if tree.getTag("AttackTreeDesigner", "Attack", "Severity").getActual().get(0).getValue() != "HIGH" :
		outputError("/errors_output/t08_09_Propagate_Severity_Update_OR_Child.err", "Expected to find Severity Tag with the value 'HIGH', instead we found the value " + attack.getTag("AttackTreeDesigner", "Attack", "Severity").getActual().get(0).getValue() + "\n")
