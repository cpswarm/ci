...
def main():
	...
	if(tree.getName() != "Tree"):
		outputError("/errors_output/t08_12_Propagate_Severity_Propagate_Grand_Parent.err", "Expected to find tree with the name 'Tree', instead we found the name " + tree.getName() + "\n")
	if tree.getTag("AttackTreeDesigner", "Attack", "Severity").getActual().get(0).getValue() != "HIGH" :
		outputError("/errors_output/t08_12_Propagate_Severity_Propagate_Grand_Parent.err", "Expected to find Severity Tag with the value 'HIGH', instead we found the value " + attack.getTag("AttackTreeDesigner", "Attack", "Severity").getActual().get(0).getValue() + "\n")
