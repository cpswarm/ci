...
def main():
	...
	testPackage = findPackage(modelPackage, "t08_Severity_Tag", "t08_04_Update_Severity_MEDIUM")
	...
	if(tree.getName() != "Tree"):
		outputError("/errors_output/t08_04_Update_Severity_MEDIUM.err", "Expected to find tree with the name 'Tree', instead we found the name " + tree.getName() + "\n")
	if tree.getTag("AttackTreeDesigner", "Attack", "Severity").getActual().get(0).getValue() != "MEDIUM" :
		outputError("/errors_output/t08_04_Update_Severity_MEDIUM.err", "Expected to find Severity Tag with the value 'MEDIUM', instead we found the value " + attack.getTag("AttackTreeDesigner", "Attack", "Severity").getActual().get(0).getValue() + "\n")
