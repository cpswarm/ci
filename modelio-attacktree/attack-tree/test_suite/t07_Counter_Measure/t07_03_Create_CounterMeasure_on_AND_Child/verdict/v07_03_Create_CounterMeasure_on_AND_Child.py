...
def main():
	...
	attackNote = attack.getDescriptor().get(0)
	...
	if attack.getTag("AttackTreeDesigner", "Attack", "Countered attack").getActual().get(0).getValue() != "true" :
		outputError("/errors_output/t07_03_Create_CounterMeasure_on_AND_Child.err", "Expected to find attack with the Countered attack having the value 'true', instead found  " + attack.getTag("AttackTreeDesigner", "Attack", "Countered attack").getActual().get(0).getValue() + "\n")
	if attack1.getTag("AttackTreeDesigner", "Attack", "Countered attack").getActual().get(0).getValue() != "false" :
		outputError("/errors_output/t07_03_Create_CounterMeasure_on_AND_Child.err", "Expected to find attack1 with the Countered attack having the value 'false', instead found  " + attack1.getTag("AttackTreeDesigner", "Attack", "Countered attack").getActual().get(0).getValue() + "\n")
	...
	if diagramHandle.getDiagramGraphics(attack).get(0).getFillColor() != "220,250,210" :
		outputError("/errors_output/t07_03_Create_CounterMeasure_on_AND_Child.err", "Expected attack to have color 220,250,210 ! Instead we found the color " + diagramHandle.getDiagramGraphics(attack).get(0).getFillColor() + "\n")
