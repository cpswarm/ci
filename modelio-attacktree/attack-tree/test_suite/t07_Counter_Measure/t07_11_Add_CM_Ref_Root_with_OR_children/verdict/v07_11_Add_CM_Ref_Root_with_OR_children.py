...
def main():
	...
	if attack1_Tree1.getDescriptor().isEmpty() :
		outputError("/errors_output/t07_11_Add_CM_Ref_Root_with_OR_children.err", "Expected to find note  in attack1_tree1 \n")

	tree1Note = attack1_Tree1.getDescriptor().get(0)
	...
	if not (tree1Note.getModel().getName()=="Counter-Measure" and tree1Note.getContent()=="Counter-Measure"):
		outputError("/errors_output/t07_11_Add_CM_Ref_Root_with_OR_children.err", "Expected to find note with the name and content 'Counter-Measure', instead found name " + tree1Note.getModel().getName() + " and content " + tree1Note.getContent() + "\n")

	# Verify Countered Tag value
	if tree.getTag("AttackTreeDesigner", "Attack", "Countered attack").getActual().get(0).getValue() != "false" :
		outputError("/errors_output/t07_11_Add_CM_Ref_Root_with_OR_children.err", 
	...
	if attack1_Tree1.getTag("AttackTreeDesigner", "Attack", "Countered attack").getActual().get(0).getValue() != "true" :
		outputError("/errors_output/t07_11_Add_CM_Ref_Root_with_OR_children.err", 
		"Expected to find attack1_Tree1 with the Countered attack having the value 'true', instead found  " + attack1_Tree1.getTag("AttackTreeDesigner", "Attack", "Countered attack").getActual().get(0).getValue() + "\n")
	...
	if diagramHandle1.getDiagramGraphics(attack1_Tree1).get(0).getFillColor() != "220,250,210" :
		outputError("/errors_output/t07_11_Add_CM_Ref_Root_with_OR_children.err", 
		"Expected attack1_Tree1 to have color 220,250,210 ! Instead we found the color " + diagramHandle1.getDiagramGraphics(attack1_Tree1).get(0).getFillColor() + "\n")
