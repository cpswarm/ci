...
def main():
	...
	testPackage = findPackage(modelPackage, "t06_Connection_Dependency", "t06_06_Delete_Dep_Root_AND_OR")
	...
	if tree.getDependsOnDependency().isEmpty():
		outputError("/errors_output/t06_06_Delete_Dep_Root_AND_OR.err", "Expected Tree to have child \n")
	
	dependency = tree.getDependsOnDependency().get(0)
	andNode = dependency.getDependsOn()

	if not andNode.getDependsOnDependency().isEmpty():
