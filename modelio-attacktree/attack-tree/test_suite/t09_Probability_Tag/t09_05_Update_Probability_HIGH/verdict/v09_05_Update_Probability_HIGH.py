...
def main():
	...
	testPackage = findPackage(modelPackage, "t09_Probability_Tag", "t09_05_Update_Probability_HIGH")
	...
	if(tree.getName() != "Tree"):
		outputError("/errors_output/t09_05_Update_Probability_HIGH.err", "Expected to find tree with the name 'Tree', instead we found the name " + tree.getName() + "\n")
	if tree.getTag("AttackTreeDesigner", "Attack", "Probability").getActual().get(0).getValue() != "HIGH" :
		outputError("/errors_output/t09_05_Update_Probability_HIGH.err", "Expected to find Probability Tag with the value 'HIGH', instead we found the value " + attack.getTag("AttackTreeDesigner", "Attack", "Probability").getActual().get(0).getValue() + "\n")
