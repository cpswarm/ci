...
def main():
	...
	testPackage = findPackage(modelPackage, "t09_Probability_Tag", "t09_08_Propagate_Probability_Update_AND_Child")
	...
	if(tree.getName() != "Tree"):
		outputError("/errors_output/t09_08_Propagate_Probability_Update_AND_Child.err", "Expected to find tree with the name 'Tree', instead we found the name " + tree.getName() + "\n")
	if tree.getTag("AttackTreeDesigner", "Attack", "Probability").getActual().get(0).getValue() != "LOW" :
		outputError("/errors_output/t09_08_Propagate_Probability_Update_AND_Child.err", "Expected to find Probability Tag with the value 'LOW', instead we found the value " + attack.getTag("AttackTreeDesigner", "Attack", "Probability").getActual().get(0).getValue() + "\n")
