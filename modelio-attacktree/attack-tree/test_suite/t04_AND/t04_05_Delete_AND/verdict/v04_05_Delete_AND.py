...
def main():
	...
	if(tree.getOwnedElement().isEmpty()):
		outputError("/errors_output/t04_05_Delete_AND.err", "Expected to find OR node, instead we found only Root \n")
	else: 
		orNode = tree.getOwnedElement().get(0)
		for node in orNode.getOwnedElement():
			if not node.getOwnedElement().isEmpty():
				outputError("/errors_output/t04_05_Delete_AND.err", "Expected to find empty nodes \n")
