...
def main():
	...
	testPackage = findPackage(modelPackage, "t04_AND", "t04_03_Create_AND_with_two_children")
	...
	if(tree.getOwnedElement().isEmpty()):
		outputError("/errors_output/t04_03_Create_AND_with_two_children.err", "Expected to find AND node, instead we found only Root \n")
	else: 
		andNode = tree.getOwnedElement().get(0)
		if(andNode.getName() != "AND"):
			outputError("/errors_output/t04_03_Create_AND_with_two_children.err", "Expected to find node with the name 'AND', instead we found the name " + andNode.getName() + "\n")
		if(andNode.getMClass().getName() != "Class"):
			outputError("/errors_output/t04_03_Create_AND_with_two_children.err", "Expected to find a the meta class Class, instead we found " + andNode.getMClass().getName() + "\n")
		if(andNode.getExtension().get(0).getName() != "AND"):
			outputError("/errors_output/t04_03_Create_AND_with_two_children.err", "Expected to find a stereotype with the name 'AND', instead we found the name " + andNode.getExtension().get(0).getName() + "\n")
		diagramService = Modelio.getInstance().getDiagramService()
		diagramHandle = diagramService.getDiagramHandle(diagram)
		if(diagramHandle.getDiagramGraphics(andNode).isEmpty()) :
			outputError("/errors_output/t04_03_Create_AND_with_two_children.err", "The created AND node is not represented in the diagram !\n")
		
		if(andNode.getOwnedElement().isEmpty()):
			outputError("/errors_output/t04_03_Create_AND_with_two_children.err", "Expected to find children of 'AND' node \n")
		else :
			attack1 = andNode.getOwnedElement().get(0)
			attack2 = andNode.getOwnedElement().get(1)
			if(attack1.getExtension().get(0).getName() != "Attack"):
				outputError("/errors_output/t04_03_Create_AND_with_two_children.err", "Expected to find a stereotype with the name 'Attack', instead we found the name " + andNode.getExtension().get(0).getName() + "\n")
