...
public class EmergencyExitROS extends AbstractSingleProblem {
	...
	*/
	...
	public double evaluateCandidate(AbstractRepresentation candidate) {
		...
		sim = new simROS(candidate, getProperties().get("rosPath").getValue(), getProperties().get("rosWs").getValue(), getProperties().get("rosPackage").getValue(), data.getRequiredNumberOfOutputs());
